...
int ReadMatrixFromBinaryFile(char* matrixName, long long &nz, long long &N,
	...
	countRead = fread(colInt, sizeof(int), nz, matrixFile);
	for (int i = 0; i < nz; i++)
		(*col)[i] = (long long)colInt[i];

	countRead = fread(tempIndx, sizeof(int), N + 1, matrixFile);
	if (countRead != N + 1)
	{
		error = 1;
		printf("Error open files %s\n", matrixName);
	...
	for (int i = 0; i < N + 1; i++)
		(*indx)[i] = (long long)tempIndx[i];

	countRead = fread(*value, sizeof(double), nz, matrixFile);
	if (countRead != (*indx)[N])
	{
		error = 1;
		printf("Error open files %s\n", matrixName);
...
void gaussBackBlockLowFull(long long startNode, long long finishNode, double** y,
	...
	double* valLCCS, long long* rowLCCS, long long* indxLCCS, long long N, long long M) {
	long long j, k, index_top, index_low;
	double sum;
	long long m = 1;
...
#pragma omp parallel for private(m, sum)
	for (long long l = 0; l < M; l++) {
		y[l][startNode] = b[l][startNode] / valLCRS[indxLCRS[startNode + 1] - 1];
		m = 1;
		for (long long k = startNode + 1; k < finishNode; k++) {// по строкам нода
			sum = 0;
			index_top = indxLCRS[k + 1] - 1;
			index_low = indxLCRS[k + 1] - 1 - m;
...
#pragma omp parallel for private(m, sum)
	for (long long l = 0; l < M; l++) {
		x[l][startNode - 1] = y[l][startNode - 1] / valUCRS[indxUCRS[startNode - 1]];
		m = 2;
		for (long long k = startNode - 2; k >= finishNode; k--) {// по строкам нода
			sum = 0;
			index_top = indxUCRS[k] + m;
			index_low = indxUCRS[k] + 1;
...
void changeBLowCCS(double *MatrixLowVal, long long *MatrixLowRow, long long *MatrixLowIndx,
	...
	for (long long j = SNodesLowl; j < SNodesLowl_1; j++) {//по всем столбам
		for (long long i = MatrixLowIndx[j] + SNodesLowl_1 - SNodesLowl - m; i < MatrixLowIndx[j + 1]; i++) {//вдоль столбца
...
void changeBUpCCS(double *MatrixUpVal, long long *MatrixUpRow, long long *MatrixUpIndx,
	...
	for (long long j = SNodesUpl_1; j < SNodesUpl; j++) {//по всем столбам
		for (long long i = MatrixUpIndx[j]; i < MatrixUpIndx[j + 1] - m; i++) {//вдоль столбца
...
void changeBLowFull(double *MatrixLowVal, long long *MatrixLowRow, long long *MatrixLowIndx,
	...
	for (int k = 0; k < M; k++) {
		for (long long j = SNodesLowl; j < SNodesLowl_1; j++) {//по всем столбам
			for (long long i = MatrixLowIndx[j]; i < MatrixLowIndx[j + 1]; i++) {//вдоль столбца
...
void changeBUpFull(double *MatrixUpVal, long long *MatrixUpRow, long long *MatrixUpIndx,
	...
	for (int k = 0; k < M; k++) {
		for (long long j = SNodesUpl_1; j < SNodesUpl; j++) {//по всем столбам
			for (long long i = MatrixUpIndx[j]; i < MatrixUpIndx[j + 1]; i++) {//вдоль столбца
