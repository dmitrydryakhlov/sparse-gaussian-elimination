...
int main(int argc, char* argv[]) {
	...
	long long *I, *IU, *IL, *J, *JU, *JL, *colU, *colL, *indxL, *indxU, *indxBinL, *indxBinU, *colBinL, *colBinU;
	double *valCOO, *valUCOO, *valLCOO, *valUCRS, *valLCRS, *valLCCS, *valUCCS, *valBinL, *valBinU;
...
#pragma omp parallel for
	...
	BlockTStart = clock();
	for (int i = 0; i < M; i++) {
		blockSolverLowCRS(valLCRS, colL, indxL, valLCCS, LowRowCCS, LowIndxCCS, SNodesLow, NodesNLow, yBlock[i], bBlock[i], N);
		blockSolverUpCRS(valUCRS, colU, indxU, valUCCS, UpRowCCS, UpIndxCCS, SNodesUp, NodesNUp, xBlock[i], yBlock[i], N);
	}
	BlockTFinish = clock();
